...
public struct ListGiftCardActivities: SquareAPIEndpoint {
	...
	public struct Params {
		...
		///   - type: If a [type](https://developer.squareup.com/reference/square_2022-08-23/objects/GiftCardActivityType) is provided, the endpoint returns gift card activities of the specified type.  Otherwise, the endpoint returns all types of gift card activities.
...
/// Creates a gift card activity to manage the balance or state of a [gift card](https://developer.squareup.com/reference/square_2022-08-23/objects/GiftCard).  For example, you create an `ACTIVATE` activity to activate a gift card with an initial balance  before the gift card can be used.
